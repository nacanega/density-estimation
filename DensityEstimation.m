...
% Set Data Matrices
Pmati = P0*eye(nStates);
Rmati = covMeas.*eye(obs);
Hmati = [eye(obs),zeros(obs,nSatSt-obs)];
modMat = zeros(nModSt);
if strcmp(Qtype,"SNC")
    Qmati = covState.*eye(nStates);
else
    Qmati = eye(obs).*(Hmati*covState);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Flags and Options
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
X_est0 = X0 + stateNoise(Xnl,sigmaState,sigmaMod,nSats,nSatSt);
dx_est0 = zeros(nStates,1);
P_0 = blockMat(nSats,Pmati);
...
[filtered,smoothed,add_info] = KF_RTSpre(t,zs,initConds,dataMats,sysFuncs,opts);
