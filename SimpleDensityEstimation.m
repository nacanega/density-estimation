...
if runCalc 
    ...
    % For H matrix
    states = ["r_x","r_y","r_z","v_x","v_y","v_z","rho_0","h_0","H"];
    ...
    % Set Data Matrices
    Qdata = 1e-6*[covPos;covVel;covMod].*speye(nStates);
    Rdata = covMeas.*speye(obs);
    Hdata = [eye(3),zeros(3),zeros(3)];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    X_est0 = squeeze(Xnl(1,:,:)).' ...
        + [sigmaPos;sigmaVel;sigmaMod].'.*randn(nSats,nStates);
...
if geneFig && (runCalc || fileLoaded)
    ...
    for sN = 1
      ...
      if strcmp(opts.outIter,"last")
          plot3(satSol(sN).filtered.X(:,1), ...
                satSol(sN).filtered.X(:,2), ...
                satSol(sN).filtered.X(:,3), ...
                "--","DisplayName","First Filter")
          plot3(satSol(sN).smoothed.X(:,1), ...
                satSol(sN).smoothed.X(:,2), ...
                satSol(sN).smoothed.X(:,3), ...
          ...
          plot3(satSol(sN).filtered.X{1}(:,1), ...
                satSol(sN).filtered.X{1}(:,2), ...
                satSol(sN).filtered.X{1}(:,3), ...
                "--","DisplayName","First Filter")
          plot3(satSol(sN).smoothed.X{1}(:,1), ...
                satSol(sN).smoothed.X{1}(:,2), ...
                satSol(sN).smoothed.X{1}(:,3), ...
                "--","DisplayName","First Smoother")
          plot3(satSol(sN).filtered.X{end}(:,1), ...
                satSol(sN).filtered.X{end}(:,2), ...
                satSol(sN).filtered.X{end}(:,3), ... 
                ":","DisplayName","Last Filter")
          plot3(satSol(sN).smoothed.X{end}(:,1), ...
                satSol(sN).smoothed.X{end}(:,2), ...
                satSol(sN).smoothed.X{end}(:,3), ...
                ":","DisplayName","Last Smoother")
      end
        hold off
