function [filSol,smoSol,addOut] = LKF_RTSstep(t,zs,initConds,dataMats,sysFuncs,opts)
...
while  maxDiff > tol && iter < maxIter && numInc < maxInc
    ...
    for i = 2:N
        ...
        if isempty(odeOpts.MaxStep)
            odeOpts.MaxStep = t(i) - t(i-1);
        end
        [~,XpredsPhis] = vsIntFun(dotFunc,[t(i-1),(t(i-1)+t(i))/2,t(i)],XpredPhi0,odeOpts);
        ...
        % Extract
        X_preds(i,:) = XpredsPhis(end,1:M);
        rPhis = XpredsPhis(end,M+1:M+M*M);
        Phis(:,:,i) = reshape(rPhis,M,M);
        XpredPhi0(1:M) = X_preds(i,:);
    end
    ...
    assert(~anynan(X_ests),"Filter Not Returning Full Vector","Filter")
    % Rauch–Tung–Striebel Smoother
    [X_sms,dx_sms,P_sms] = RTSpre(X_preds,dx_ests,Phis,P_ests,P_preds);
    assert(~anynan(X_sms),"Smoother Not Returning Full Vector","Smoother")
