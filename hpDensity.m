function [rho,rho_m_low,rho_M_low,H_min,H_max] = hpDensity(h,chi_term)
...
H_min = (h_low - h_high) ./ log(rho_m_high ./ rho_m_low);
H_max = (h_low - h_high) ./ log(rho_M_high ./ rho_M_low);

% Exponential interpolation for current altitude
rho_min = rho_m_low .* exp((h_low - h) ./ H_min);
rho_max = rho_M_low .* exp((h_low - h) ./ H_max);
